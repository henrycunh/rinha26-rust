...
RISKY_MCCS = {"5944", "7801", "7802", "7995"}
...
def fast_path_score(request: dict) -> int | None:
    transaction = request["transaction"]
    customer = request["customer"]
    merchant = request["merchant"]
    terminal = request["terminal"]
    ...
    if (
        amount <= 500.0
        and amount_ratio <= 0.50001
        and installments <= 3
        and tx_count_24h <= 5
        and merchant_known
        and km_from_home <= 50.0
        and mcc in SAFE_MCCS
    ):
        return 0
    ...
    if (
        ...
        and installments >= 5
        and tx_count_24h >= 6
        and not merchant_known
        and km_from_home >= 150.0
        and mcc in RISKY_MCCS
    ):
        return 5
...
def load_training_data(path: Path, residual_fast_path: bool) -> tuple[np.ndarray, np.ndarray, str]:
    raw = path.read_bytes()
    payload = json.loads(raw)
    entries = payload["entries"]
    if residual_fast_path:
        entries = [entry for entry in entries if fast_path_score(entry["request"]) is None]
...
def main() -> int:
    ...
    parser.add_argument(
        "--residual-fast-path",
        action="store_true",
        help="train only on entries not decided by the runtime deterministic fast path",
    )
    parser.add_argument("input", type=Path)
    parser.add_argument("output", type=Path)
    args = parser.parse_args()

    features, labels, checksum = load_training_data(args.input, args.residual_fast_path)
